...
class MyCustomModel(nn.Module):
    def __init__(self, base_model, n_classes, dropout=0.05, cuda_0=CUDA_0, cuda_1=CUDA_1, cuda_2=CUDA_2):
        ...
        self.myvlad = nn.Sequential(
            ...
            NextVLAD(num_clusters=128, dim=2048, expansion=4, num_class=n_classes)
    ...
    def forward(self, input_, **kwargs):
        ...
        hidden_states = self.base_model(X.to('cuda:' + str(self.cuda_0)), attention_mask=attention_mask.to('cuda:' + str(self.cuda_0))).last_hidden_state
        ...
        lstm_out = nn.functional.leaky_relu(lstm_out[:, :, :])
        vlad_out = self.myvlad(lstm_out.to('cuda:' + str(self.cuda_2)))
        myo = nn.functional.softmax(vlad_out, dim=1)
        return myo
