...
class MyCustomModel(nn.Module):
    def __init__(self, base_model, n_classes, dropout=0.05, cuda_0=CUDA_0, cuda_1=CUDA_1, cuda_2=CUDA_2):
        super().__init__()
        self.cuda_0 = cuda_0
        self.cuda_1 = cuda_1
        self.cuda_2 = cuda_2
        self.base_model = base_model.to(self.cuda_0)
        ...
        '''
        self.mypool = nn.AvgPool1d(4)
        self.final = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(dropout),
            nn.Linear((512*BERT_EMB)//4, BERT_EMB),
            nn.LeakyReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, input_, **kwargs):
        ...
        hidden_states = self.base_model(X.to(self.cuda_0), attention_mask=attention_mask.to(self.cuda_0)).last_hidden_state
        bert_tokens = hidden_states[:, :, :]
        lstm_out, (hn, cn) = self.mylstm(bert_tokens.to(self.cuda_1))
        lstm_out = nn.functional.leaky_relu(lstm_out[:, :, :BERT_EMB] + lstm_out[:, :, BERT_EMB:])
        # vlad_out = self.myvlad(lstm_out.to(self.cuda_2))
        pooling_out = self.mypool(lstm_out.to(self.cuda_1))
        myo = self.final(pooling_out.to(CUDA_2))
